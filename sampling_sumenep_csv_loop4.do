...
replace selected_c=1 if selected_c>0
gsample 1 [w=n] if selected_c==0
replace selected_c=5
append using `s_cluster'
*drop selected_c
*gen selected_c=_n
tempfile s_cluster_add
save `s_cluster_add', replace 
restore
merge m:1 iddesa using `s_cluster_add'
replace selected_c=0 if selected_c==.
drop _merge
drop n
